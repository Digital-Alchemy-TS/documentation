...
| `getIntervalMs(offset)` | Parse a `TOffset` string to milliseconds |
| `getIntervalTarget(offset)` | Parse a `TOffset` string to a target `Dayjs` datetime |
| `relativeDate(past, future?)` | Human-readable relative time: "3 minutes ago" |
